...
- `picker.ts`, `prompts.ts`, `startup-menu.tsx` — interactive provider/model selection (line-based and Ink-based variants).
...
- `text-tool-parser.ts` — fallback recovery of tool calls from prose when a model emits XML/JSON in plain text.
- `tool-call-corrector.ts` — LLM-driven correction of malformed tool calls.
...
- `config.ts`, `config-types.ts` — config file load/merge/save with zod-validated schema.
...
Tool execution is gated by `src/permissions.ts` (allow-once / allow-always / deny / domain whitelist) and the security policies in `src/security/` (path jail, env scrubbing, bash rule matching).
...
### `src/ui/`
Two render targets. **`ink/`** is the React + Ink TUI: `App.tsx` is the tab host, `Session.tsx` is one tab's REPL, `use-agent-loop.ts` is the React hook that drives the loop, `agent-loop/` contains DI'd helpers (init, history, run-loop, event-handler), `slash-commands.ts` + `slash/` dispatch user-typed slash commands, `tabs/` provides tab registry/context.
...
7. When the model emits `tool_calls`, `agent/run-tool-calls.ts` is invoked: each call is permission-checked (`src/permissions.ts`), security-checked (`src/security/`), executed, and the result appended to the conversation as a `tool` message.
...
| New slash command | `src/ui/ink/slash/<name>.ts` + `src/ui/ink/slash-commands.ts` (dispatcher) |
