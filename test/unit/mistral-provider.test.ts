...
describe('MistralProvider', () => {
  it('lists chat-capable models from the models endpoint', async () => {
    await withServer((req, res) => {
      ...
      res.end(JSON.stringify({
        data: [
          { id: 'mistral-small-latest', max_context_length: 128000, capabilities: { completion_chat: true, function_calling: true } },
  ...
  it('builds picker details from model metadata', async () => {
    await withServer((_req, res) => {
      ...
      res.end(JSON.stringify({
        data: [
          { id: 'magistral-medium-latest', max_context_length: 128000, capabilities: { completion_chat: true, function_calling: true } },
        ],
      }));
    }, async (baseUrl) => {
      const provider = new MistralProvider({ token: 'test-token', host: baseUrl });
  ...
  it('uses the Codestral base URL and provider name', async () => {
    await withServer((req, res) => {
      ...
      res.end(JSON.stringify({
        data: [
          { id: 'codestral-latest', max_context_length: 256000, capabilities: { completion_chat: true, function_calling: true } },
        ],
      }));
    }, async (baseUrl) => {
