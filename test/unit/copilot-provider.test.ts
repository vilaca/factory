...
describe('CopilotProvider', () => {
  ...
  it('lists models from /models using bearer auth', async () => {
    await withServer((req, res) => {
      ...
      res.end(JSON.stringify({
        data: [
          { id: 'gpt-4.1', capabilities: { type: 'chat' }, model_picker_enabled: true, policy: { state: 'enabled' } },
          { id: 'claude-sonnet-4', capabilities: { type: 'chat' }, model_picker_enabled: true, policy: { state: 'enabled' } },
          { id: 'copilot/claude-opus-4.7', capabilities: { type: 'chat' }, model_picker_enabled: false, policy: { state: 'enabled' } },
          { id: 'text-embedding-3-large', capabilities: { type: 'embeddings' }, model_picker_enabled: true, policy: { state: 'enabled' } },
          { id: 'retired-model', capabilities: { type: 'chat' }, model_picker_enabled: true, policy: { state: 'disabled' } },
        ],
      }));
    }, async (baseUrl) => {
  ...
  it('coalesces sparse streamed tool call indices without yielding holes', async () => {
    await withServer((req, res) => {
      ...
      res.write('data: {"choices":[{"delta":{"tool_calls":[{"index":1,"id":"call_2","function":{"name":"Read","arguments":"{\\"file_path\\":\\"README.md\\"}"}}]}}]}\n\n');
      res.write('data: {"choices":[{"finish_reason":"tool_calls"}],"usage":{"prompt_tokens":11,"completion_tokens":7,"total_tokens":18}}\n\n');
      res.write('data: [DONE]\n\n');
      res.end();
    }, async (baseUrl) => {
      const provider = new CopilotProvider({ token: 'test-token', host: baseUrl });
