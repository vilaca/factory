...
describe('OpenRouterProvider', () => {
  ...
  it('appends (free) to free model display names', async () => {
    await withServer((_req, res) => {
      ...
      assert.strictEqual(provider.getDisplayModelName('openai/gpt-4.1:free'), 'openai/gpt-4.1 (free)');
      assert.strictEqual(provider.getDisplayModelName('anthropic/claude-sonnet-4'), 'anthropic/claude-sonnet-4');
    });
  });
  ...
  it('builds picker details and warnings from model metadata', async () => {
    await withServer((_req, res) => {
      ...
      assert.deepStrictEqual(provider.getModelPickerInfo('openai/gpt-4.1:free'), {
        ...
        detail: 'vision · tools · reasoning · structured output · max 65.5k out · free 40k TPM · free 200 RPD · free 10 RPM',
        warning: undefined,
      });
