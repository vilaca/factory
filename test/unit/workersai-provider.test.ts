  ...
  it('lists text-generation models from model search using bearer auth', async () => {
    await withServer(
      (req, res) => {
        ...
        assert.strictEqual(
          req.url,
          '/client/v4/accounts/test-account/ai/models/search?page=1&per_page=100&hide_experimental=false',
        );
    ...
    await withServer(
      ...
      async baseUrl => {
        ...
        assert.deepStrictEqual(
          provider.getModelPickerInfo('@cf/meta/llama-4-scout-17b-16e-instruct'),
          {
