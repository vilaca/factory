    ...
    await withServer((_req, res) => {
      ...
      res.end(JSON.stringify({
        models: [
          ...
          { name: 'command-r7b-12-2024', endpoints: ['chat'], context_length: 128000, is_deprecated: true },
        ],
      }));
    }, async (baseUrl) => {
      const provider = new CohereProvider({ token: 'test-token', host: baseUrl });
  ...
  it('sends Cohere chat payloads and parses tool calls', async () => {
    await withServer((req, res) => {
      ...
      req.on('end', () => {
        ...
        assert.strictEqual(parsed.messages[1].tool_calls[0].function.arguments, '{"file_path":"README.md"}');
      ...
      const result = await provider.chatNoStream(
        ...
        [
          { role: 'user', content: 'inspect the repo' },
          {
            ...
            tool_calls: [{ id: 'call_1', function: { name: 'Read', arguments: { file_path: 'README.md' } } }],
          },
