...
describe('parseRotationEntry', () => {
  it('parses a simple <provider>:<model>', () => {
    assert.deepStrictEqual(
      parseRotationEntry('anthropic:claude-haiku-4-5'),
  ...
  it('splits on the first colon — model may contain colons', () => {
    assert.deepStrictEqual(
      parseRotationEntry('workersai:@cf/llama:variant-1'),
      { provider: 'workersai', model: '@cf/llama:variant-1' },
    );
  });
...
describe('parseRotationChain', () => {
  it('parses a comma-separated list', () => {
    assert.deepStrictEqual(
      parseRotationChain('anthropic:claude-haiku-4-5,groq:llama-3.3-70b'),
      [
        { provider: 'anthropic', model: 'claude-haiku-4-5' },
  ...
  it('skips empty entries from trailing commas', () => {
    assert.deepStrictEqual(
      parseRotationChain('anthropic:a,,groq:b,'),
      [
