...
export class CerebrasProvider implements Provider {
  ...
  async *chat(
    ...
    yield* streamOpenAiChat({
      ...
      body: buildChatBody({ model, messages, tools, stream: true, options, providerName: 'Cerebras' }),
  ...
  async chatNoStream(
    ...
    return sendOpenAiChat({
      url: `${this.baseUrl}/chat/completions`,
      headers: this.authHeaders(),
      body: buildChatBody({ model, messages, tools, stream: false, options, providerName: 'Cerebras' }),
      signal: options?.signal,
      providerName: 'Cerebras',
    });
  }
