...
import { renderWelcome, animateLogo } from '../../ui/renderer.js';
import { parseArgs, printUsage, printVersion } from '../args.js';
import {
  probeAllProviders,
  resolveCredentialsFor,
  type StartupCredentials,
} from '../auth/index.js';
import { buildExperimentalConfig } from './config.js';
import {
  applyRotationPhase,
...
// eslint-disable-next-line max-statements, complexity -- runMain() is the orchestrator: each branch is a one-liner phase call. The remaining statements are sequential await calls that don't benefit from further splitting.
export async function runMain(): Promise<void> {
  ...
  // Startup-time validation: every declared prerequisite must reference
  // a registered tool. Catches typos as a clean startup error instead of
  // a confusing prerequisite_nudge mid-run. Must run after MCP and
  // subagent registration so dynamically-added tools are in scope.
  validatePrereqReferences(toolRegistry.getDefinitions());
