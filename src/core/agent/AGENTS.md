...
3. **`finalizeTurn` owns the exit triplet.** Every `done` outcome funnels through `yield* finalizeTurn(options, turnsUsed, lastUsage, outcome)`, which yields the `error` event (if any), fires the Stop hook, and yields `turn-complete`. The outer try/catch in `runAgent` is no exception — it constructs the `TurnExit` itself (the abort-vs-error classification has to happen before any hook fires) and then routes through `finalizeTurn`. The only branch in `runAgent` that bypasses `finalizeTurn` is the pre-userInput abort check at the very top of the function: no turn has started, so no stop hook should fire and `turnsUsed` is 0.
