...
- **Don't bypass `PathPolicy` / `EnvPolicy` plumbing** by calling `process.cwd()` / `process.env` inside a security function. _Enforced by arch test:_ `test/unit/arch/modularity.test.ts` — "src/security/\*\* must read policy snapshots, not process.cwd() / process.env directly". The whole point of taking the policy as an argument is that each tool call sees a frozen snapshot bound to its turn / tab; reading process state breaks the snapshot boundary.
